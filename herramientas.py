...
#Convertimos las variables de una columna de tipo str a tipo tiempo.
...
#Verificamos y mostramos filas duplicadas en un Df basado en una columna específica.
...
def accidentes_mes(df):
    ...
    for i, year in enumerate(años):
        ...
        ax.lines[0].set_color('#800080')  # Cambia el color de la línea del gráfico
...
def meses_con_mas_accidentes(df):
    ...
    plt.bar(data_meses['Mes'], data_meses['Cantidad de Accidentes'], color='#9E79DB')
    plt.title('Meses con Mayor Cantidad de Accidentes')
    plt.xlabel('Mes')
    plt.ylabel('Cantidad de Accidentes')
    plt.xticks(rotation=45)
    plt.grid(axis='y', color='#9400D3', linestyle='--', linewidth=0.5)
...
def cantidad_accidentes_tiempo(df):
    ...
    ax = sns.barplot(x='Categoria tiempo', y='Cantidad accidentes', data=data, color='#9E79DB')
    ...
    for index, row in data.iterrows():
        ax.annotate(f'{row["Cantidad accidentes"]}', (index, row["Cantidad accidentes"]), ha='center', va='bottom', color='#000000')
...
def accidentes_en_calle(df):
    # Se crea el gráfico
    fig, axes = plt.subplots(1, 2, figsize=(10, 4))
    
    sns.countplot(data=df, x='Tipo de calle', ax=axes[0], color='#9E79DB')
    axes[0].set_title('Cantidad de víctimas por tipo de calle') ; axes[0].set_ylabel('Cantidad de víctimas')

    sns.countplot(data=df, x='Cruce', ax=axes[1], color='#9E79DB')
...
def accidentes_tipo_de_vehiculo(df):
    ...
    # Se crea el gráfico de barras
    plt.figure(figsize=(10, 6))
    ax = sns.barplot(x='Tipo de Vehículo', y='Cantidad de Accidentes', data=data, color='#9E79DB')
    ...
    # Se agrega las cantidades en las barras
    for index, row in data.iterrows():
        ax.annotate(f'{row["Cantidad de Accidentes"]}', (index, row["Cantidad de Accidentes"]), ha='center', va='bottom', color='#000000')
...
def cantidad_victimas_sexo_rol_vehiculo(df):
    # Se crea el gráfico
    fig, axes = plt.subplots(1, 3, figsize=(15, 4))

    # Gráfico 1: Sexo
    sns.countplot(data=df, x='Sexo', ax=axes[0], color= '#9E79DB')
    ...
    df_rol.plot(kind='bar', stacked=True, ax=axes[1], color=['#9E79DB', '#8CDBC0'])
    ...
    df_victima.plot(kind='bar', stacked=True, ax=axes[2], color=['#9E79DB', '#8CDBC0'])
...
def victimas_rango_etario(df):
    ...
    return grupos['Edad'].size().plot.bar(figsize=(10, 6), color=['#DBCE95', '#7F78D6', '#86D0DB', '#DC7AA2', '#81D78C'],
...
def calcular_victimas(df_hechos, df_victimas):
